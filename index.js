...
async function generateAIInsights(tokenData, weeklyData, sentiment) {
  ...
  // Simulated AI insights; in production, integrate with an AI API.
  try {
    const insights = [
    ...
    metrics: {
      ...
      transactions: {
        h1: {
          ...
          ratio: tokenData.txns.h1.sells
            ? tokenData.txns.h1.buys / tokenData.txns.h1.sells
            : tokenData.txns.h1.buys
        }
      }
    },
    risk: riskAnalysis,
...
const agent = new Agent({
  ...
  apiKey: process.env.OPEN_SERV_API_KEY || "19e9744583aa48a7b02aaba24dda618f"
});
...
agent.addCapability({
  ...
  schema: z.object({
    chainId: z.string().nonempty({ message: "chainId is required" }),
    tokenAddress: z.string().nonempty({ message: "tokenAddress is required" })
  }),
  async run({ args }) {
